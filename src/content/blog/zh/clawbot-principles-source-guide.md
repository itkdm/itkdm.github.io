---
title: "Clawbot / Moltbot 原理与源码分析"
date: 2026-01-28T18:28:42+08:00
tags: ["AI", "Agent", "Clawbot", "Moltbot", "源码分析"]
summary: "从入门理解到系统原理，再到源码入口，完整拆解 Clawbot / Moltbot 为什么不只是一个聊天机器人，而是一套以 Gateway 为中心、支持多渠道、多设备和 Canvas 的个人 AI Agent 平台。"
lang: "zh"
slug: "clawbot-principles-source-guide"
draft: false
---

如果你这两年一直在用 ChatGPT、豆包之类的大模型，大概率已经踩过这些坑：
...
**Moltbot 想做的事情其实很简单：你只需要在熟悉的聊天软件里说话（WhatsApp / Telegram 等），剩下的都交给它。**
...
**Moltbot 是一个你自己掌控的“个人 AI 助手平台”**：你在一台机器上跑一个长期在线的 **Gateway（网关/控制平面）**，它接入 Telegram / WhatsApp 等对话入口，把消息交给 AI Agent 处理；必要时还能调度你的 **手机/电脑（Nodes）** 去执行拍照、录屏、定位、运行命令等动作，再把结果发回到原来的聊天窗口里。

官方文档也给出了一张类似的“how it works”架构示意：Gateway 是单一长驻进程，下面挂着 CLI、WebChat、macOS app、iOS/Android nodes 等组件，核心通信通过 WebSocket 控制平面完成（默认 loopback）：见 官网总览（How it works / Network model）[1] 与 架构概念[2]。
...
这类场景看起来像普通 bot，但它的“底盘”更强：同一个 Gateway 还可以同时接 WhatsApp/Slack/Discord，而不是每个平台各搭一套（多渠道接入的设计在 Channels 文档[3] 里有完整列举）。
...
Moltbot 的 Canvas 相当于给 Agent 一个“可视化工作台”：可以 present/navigate/eval/snapshot；还可以用 A2UI 把结构化 UI 推送到 Canvas 面板里（macOS 端文档写得很清楚：Canvas 面板是 WKWebView；Canvas 通过 Gateway WebSocket 暴露；A2UI 由 Gateway 的 canvas host 提供，默认 A2UI host URL 是 `http://<gateway-host>:18793/__moltbot__/a2ui/`，而面向节点 WebView 的 Canvas host 路径是 `/__moltbot__/canvas/`）。见 macOS Canvas[4] 与 官网总览（Network model）[1]。（代码侧路径常量在 `src/canvas-host/a2ui.ts`：`A2UI_PATH`/`CANVAS_HOST_PATH`）
...
3. 3. **Agent 读上下文，决定要不要“找设备帮忙”**

- Agent 拿到这条消息和最近的会话历史（Session 记忆）。
- 它理解到：
  > 需要在一个有 git 仓库的地方执行 `git status`
- 于是它不会在 Telegram 里“假装跑命令”，而是打算调用 **Nodes 工具**。（Nodes 通过 Gateway WS + pairing 接入的模型见 官网总览（How it works / Network model）[1] 与 Nodes 文档[5]）

4. 4. **Gateway 查当前有哪些 Node 在线（设备能力视图）**
...
6. 6. **Agent 根据结果整理回复，再让 Gateway 发回 Telegram**
...
在 macOS Canvas 文档里，官方的设计是（见 macOS Canvas[4]；Canvas host 的端口与路径也在 官网总览（Network model）[1] 有明确说明）：
...
在 Moltbot 里，至少有两条清晰的路线（整体概念在 Memory 文档[6] 里有更系统的说明）：
...
+ • 哪些工具可以在大多数情况下自由使用（如 memory\_search）。
...
如果你对某一块特别感兴趣（比如只想深挖“浏览器工具”或“WebSocket 协议格式”），可以从上面这些文件名出发继续往下钻，基本不会迷路。

这也是这篇文章想达到的效果：**先用入门和原理解锁心智模型，再用这一节把你安全地带到代码入口。**
...
`[1]` 官网总览（How it works / Network model）: *https://docs.molt.bot/*  
`[2]` 架构概念: *https://docs.molt.bot/concepts/architecture*  
`[3]` Channels 文档: *https://docs.molt.bot/channels*  
`[4]` macOS Canvas: *https://docs.molt.bot/platforms/mac/canvas*  
`[5]` Nodes 文档: *https://docs.molt.bot/nodes*  
`[6]` Memory 文档: *https://docs.molt.bot/concepts/memory*
